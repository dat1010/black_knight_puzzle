...
`);
  ...
  // js/chess_hook.js
  var columns = ["H", "G", "F", "E", "D", "C"];
  var columnIndex = (col) => columns.indexOf(col);
  var cellValue = (cell) => cell?.dataset.val ?? "";
  var isOpenCell = (cell) => cellValue(cell) === "0";
  var isBlockedCell = (cell) => cellValue(cell) === "x";
  var isPieceCell = (cell) => cell && !isBlockedCell(cell) && !isOpenCell(cell);
  var positionFor = (cell) => ({
    row: Number(cell.dataset.row),
    col: cell.dataset.col,
    val: cellValue(cell)
  });
  var pathIsClear = (board, from, to, stepCol, stepRow) => {
  ...
  var isLegalDestination = (board, fromCell, toCell) => {
    if (!fromCell || !toCell || fromCell === toCell || !isOpenCell(toCell))
      return false;
    ...
    switch (from.val) {
      ...
      case "B":
        return absCol === absRow && pathIsClear(board, from, to, Math.sign(deltaCol), Math.sign(deltaRow));
      case "R":
  ...
  var ChessHook = {
    mounted() {
      ...
      this.onPointerMove = (event) => {
        if (!this.dragging || !this.selectedCell)
          return;
      ...
      this.onPointerUp = (event) => {
        ...
        this.pointerStart = null;
        if (movedEnough && endCell)
          this.tryMoveTo(endCell);
